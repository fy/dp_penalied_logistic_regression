...
# ignore coefficients if |coef| / |largest_coef| < coef_thresh_ratio
coef_thresh_ratio = 1e-2
  ...
  for (MM in MM_vec) {
    ...
    for (epsilon in epsilon_vec) {
      for (alpha in alpha_vec) {
        result_by_lambda =
          noisy_result[[ii]][[paste(MM)]][[paste(epsilon)]][[paste(alpha)]]
...
for (MM in MM_vec) {
  for (epsilon in epsilon_vec) {
    for (alpha in alpha_vec) {
      kappa = get_kappa(MM)
      result_by_lambda =
        noisy_result[[1]][[paste(MM)]][[paste(epsilon)]][[paste(alpha)]]
  ...
  for (MM in MM_vec) {
    for (epsilon in epsilon_vec) {
      for (alpha in alpha_vec) {
        ...
        ## perturbed scores
        perturbed_score_vec = score_vec + 2 * beta * rexp(length(score_vec),
                                                          rate=epsilon)
  ...
  for (MM in MM_vec) {
    ...
    for (epsilon in epsilon_vec) {
      ...
      for (alpha in alpha_vec) {
        ...
        best_model_list[[ii]][[paste(MM)]][[paste(epsilon)]][[paste(alpha)]] =
          noisy_result[[ii]][[paste(MM)]][[paste(epsilon)]][[paste(alpha)]][[paste(best_lambda)]]
      }
    }
  }
}
  ...
  for (MM in MM_vec) {
    ...
    for (epsilon in epsilon_vec) {
      for (alpha in alpha_vec) {
        ...
        # ignore intercept
        top_coef_name = colnames(design_matrix)[-1][top_coef_idx[-1]]
      ...
      util_dtf = rbind(util_dtf,
                       c(MM=MM, epsilon=epsilon, alpha=alpha,
                         recovered_interaction=
                           mean(sub_dtf$recovered_interaction),
                         recovered_main_effcts=
                           mean(sub_dtf$recovered_main_effcts) / 2,
...
util_dtf$epsilon = factor(util_dtf$epsilon,
                          levels=levels(util_dtf$epsilon)[c(3, 4, 1, 2)])
  ...
  for (MM in MM_vec) {
    ...
    for (epsilon in epsilon_vec) {
      for (alpha in alpha_vec) {
        ...
        top_coef_idx = (abs(coef_vec) / max(abs(coef_vec)) > coef_thresh_ratio)
        top_coef_name =
          colnames(design_matrix)[-1][top_coef_idx[-1]] # ignore intercept
...
specificity_util_dtf$epsilon =
  factor(specificity_util_dtf$epsilon,
         levels=levels(specificity_util_dtf$epsilon)[c(3, 4, 1, 2)])
...
epsilon_labels = sapply(levels(util_dtf$epsilon), function(ee) {
  ...
  as.expression(substitute(paste(epsilon, "=", ee, ',  convex_min=',
                                 convex_min,  sep=''),
                           list(ee=ee, convex_min=round(convex_min, 2))))
})

barchart(recovered_interaction + recovered_main_effcts +
           recovered_all ~ alpha | epsilon,
         data=util_dtf,
...
for (MM in MM_vec) {
  ...
  for (epsilon in epsilon_vec) {
    for (alpha in alpha_vec) {
      result_by_lambda = n
      o_noise_result[[paste(MM)]][[paste(epsilon)]][[paste(alpha)]]
      for (res in result_by_lambda) {
        lambda = res$params$lambda
        score = validate.f(design_matrix, yy, res$optim_result$estimate)
        no_noise_validation_score_dtf =
          rbind(no_noise_validation_score_dtf,
                c(MM=MM, epsilon=epsilon, alpha=alpha,
                  lambda=lambda, score=score))
      }
    }
  }
}
no_noise_validation_score_dtf = as.data.frame(no_noise_validation_score_dtf,
                                              stringsAsFactors=FALSE)
no_noise_validation_score_dtf$score =
  as.numeric(no_noise_validation_score_dtf$score)
  ...
  for (MM in MM_vec) {
    ...
    for (epsilon in epsilon_vec) {
      ...
      for (alpha in alpha_vec) {
        ...
        no_noise_best_model_list[[paste(MM)]][[paste(epsilon)]][[paste(alpha)]] =
          no_noise_result[[paste(MM)]][[paste(epsilon)]][[paste(alpha)]][[paste(best_lambda)]]
      }
    }
  }
}
...
no_noise_freq_dtf$recovered_interaction =
  as.numeric(no_noise_freq_dtf$recovered_interaction)
no_noise_freq_dtf$recovered_main_effcts =
  as.numeric(no_noise_freq_dtf$recovered_main_effcts)
...
no_noise_util_dtf$recovered_interaction =
  as.numeric(no_noise_util_dtf$recovered_interaction)
no_noise_util_dtf$recovered_main_effcts =
  as.numeric(no_noise_util_dtf$recovered_main_effcts)
...
no_noise_util_dtf$epsilon =
  factor(no_noise_util_dtf$epsilon,
...
for (MM in MM_vec) {
  ...
  for (epsilon in epsilon_vec) {
    for (alpha in alpha_vec) {
      ...
      no_noise_specificity_freq_dtf =
        rbind(no_noise_specificity_freq_dtf,
              c(iter=ii, MM=MM, epsilon=epsilon, alpha=alpha,
                no_wrong_snp=no_wrong_snp))
    }
  }
...
no_noise_specificity_freq_dtf =
  as.data.frame(no_noise_specificity_freq_dtf, stringAsFactor=FALSE)
no_noise_specificity_freq_dtf$no_wrong_snp =
  as.logical(no_noise_specificity_freq_dtf$no_wrong_snp)
...
no_noise_specificity_util_dtf =
  as.data.frame(no_noise_specificity_util_dtf, stringsAsFactors=FALSE)
no_noise_specificity_util_dtf$specificity =
  as.numeric(no_noise_specificity_util_dtf$specificity)
no_noise_specificity_util_dtf$alpha =
  as.factor(no_noise_specificity_util_dtf$alpha)
no_noise_specificity_util_dtf$epsilon =
  factor(no_noise_specificity_util_dtf$epsilon)
no_noise_specificity_util_dtf$epsilon =
  factor(no_noise_specificity_util_dtf$epsilon,
...
epsilon_labels = sapply(levels(no_noise_specificity_util_dtf$epsilon),
                        function(ee) {
  convex_min = get_convex_min(no_noise_specificity_util_dtf$MM[1], NN,
                              as.numeric(ee))
  as.expression(substitute(paste('convex_min=', convex_min,  sep=''),
                           list(convex_min=round(convex_min, 2))))
})
